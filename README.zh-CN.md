...
- [x] MCP 基础接入：Claude stdio MCP 可由 Lilia 管理并注入运行时；Codex 可读取 `~/.codex/config.toml` 中的 MCP server 并传入运行时。
...
- [x] Codex MCP 读取：沿用 Codex 配置读取 MCP server，并在运行时注册。
- [ ] Codex 审批桥接（部分接入）：命令和文件变更审批可进入统一工具确认；`additionalPermissions` 和 `availableDecisions` 等增强字段还未展示或利用。
- [ ] Codex MCP 管理（部分接入）：界面可查看 MCP server 并打开 Codex 配置文件；还不能在 LiliaCode 内增删改 Codex MCP server。
- [ ] Codex 配置档案（部分接入）：每轮会传入 cwd、sandbox 和 approval policy；profiles、项目级配置和 sticky `thread/settings/update` 尚未接入。
